...
console.log('tomorrowStockPrices:', JSON.stringify(currentStockPrices));
...
function sellStock(companyName, quantityToSell, event) {
  const company = companies.find((company) => company.name === companyName);
  const stockPrice = company.price;
...
console.log(counTll+"    ??????????????????????????????????????????????????????????????????????????????  event sell   "+event);
